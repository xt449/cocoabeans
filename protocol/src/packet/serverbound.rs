...
pub mod login {
    ...
    impl StartPayload {
        pub const BUILDER: ServerBoundPacketBuilder = |mut reader| {
            Some(Box::new(Self {
                username: reader.read_limited_string(16).ok()?,
            }))
        };
    }
    ...
    impl PluginResponsePayload {
        pub const BUILDER: ServerBoundPacketBuilder = |mut reader| {
            let message_id = reader.read_varint().ok()?;
            let successful = reader.read_boolean().ok()?;
            let data;
            if successful {
                data = Some(reader.read_byte_vec(reader.remaining()).ok()?);
            } else {
                data = None;
            }
            Some(Box::new(Self {
...
pub mod play {
    ...
    pub struct InteractEntityPayload {
        ...
        pub hand: i32,             // Optional unless interaction_type is Interact or InteractAt
        pub sneeking: bool,
    }
